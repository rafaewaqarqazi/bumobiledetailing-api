...
export const CustomerRepository = AppDataSource?.getRepository(Customer).extend({
  ...
  async createCustomer(customerObj: Customer): Promise<Customer> {
    ...
    customerObj.password = bcrypt.hashSync(customerObj.password, config.hashSaltRounds);
  ...
  async updateCustomer(
    ...
    if (!Customer.hasId(customerObj)) throw new BadRequestError('Customer id not provided');
    ...
    if (customerObj.newPassword) {
      customerObj.password = bcrypt.hashSync(customerObj.newPassword, config.hashSaltRounds);
